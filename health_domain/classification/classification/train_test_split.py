...
FLAG = ''
valid_flags = ('missing_values', 'outliers', 'scaling', 'feature_selection', 'feature_extraction')
if len(sys.argv) == 2 and sys.argv[1] in valid_flags:
    FLAG = sys.argv[1]
else:
    print("Invalid format, try:  python train_test_split.py [missing_values|outliers|scaling|feature_selection|feature_extraction]")
    exit(1)
