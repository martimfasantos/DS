...
for i in range(len(file_names)):
    ...
    train.to_csv(f'data/train_and_test/missing_values/{file_name}_train.csv', index=False)

    # Test CSV
    test = concat([DataFrame(tstX, columns=data.columns), DataFrame(tstY,columns=[target])], axis=1)
    test.to_csv(f'data/train_and_test/missing_values/{file_name}_test.csv', index=False)
    ...
    savefig(f'../data_preparation/images/missing_values/distributions_train_test/{file_name}_distribution_train_test.png')
    # show()