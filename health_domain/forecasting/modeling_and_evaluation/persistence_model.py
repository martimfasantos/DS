...
if len(sys.argv) == 2 and sys.argv[1] in valid_flags:
    ...
    print("Invalid format, try:  python persistence_model.py [original|aggregation|smoothing|differentiation]")
    exit(1)
...
for i in range(len(file_names)):
    ...
    # Train
    if FLAG == 'original':
        train = read_csv('../analysis_and_preparation/data/train_and_test/glucose_train.csv', index_col=index_col, sep=',', decimal='.', parse_dates=True, infer_datetime_format=True, dayfirst=True)
    else:
