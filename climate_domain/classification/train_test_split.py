...
FLAG = ''
valid_flags = ('outliers', 'scaling', 'feature_selection', 'feature_extraction', 'feature_generation')
if len(sys.argv) == 2 and sys.argv[1] in valid_flags:
    FLAG = sys.argv[1]
else:
    print("Invalid format, try:  python train_test_split.py [outliers|scaling|feature_selection|feature_extraction|feature_generation]")
    exit(1)
