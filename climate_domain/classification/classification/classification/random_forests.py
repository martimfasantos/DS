...
from ds_charts import plot_evaluation_results, multiple_line_chart, horizontal_bar_chart, HEIGHT, plot_overfitting_study_rf
from sklearn.metrics import accuracy_score


file_tag = 'drought'
file_name = f'{file_tag}_under' # TODO might change
...
for k in range(len(max_depths)):
    ...
    for f in max_features:
        ...
        for n in n_estimators:
            rf = RandomForestClassifier(n_estimators=n, max_depth=d, max_features=f, n_jobs=3)
            rf.fit(trnX, trnY)
...
savefig(f'images/random_forests/{file_tag}_rf_study.png')
# show()
...
savefig(f'images/random_forests/{file_tag}_rf_best.png')
# show()
...
savefig(f'images/random_forests/{file_tag}_rf_ranking.png')
...
plot_overfitting_study_rf(n_estimators, y_trn_values, y_tst_values, name=f'RF_depth={max_depth}_vars={f}', xlabel='nr_estimators', ylabel=str(eval_metric))